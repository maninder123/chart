...
function legend( svg, data ) {
    var legendHeight = ( h < 480 ) ? 7 : 15,
        legendWidth =  ( w < 480 ) ? 15 : 30,
...
//                         if(i==3)
//                 { var xTranslate = ( width * 0.3 ),  yTranslate = (margin*0.7) }
//                 else
//                    { xTranslate = ( width * 0.5*i ),  yTranslate = (margin*0.7) }
//                    //var yTranslate = ( w < 768 ) ? ( width*2  * ( 0.09 * i ) ) : ( width * 0.09 );
                return "translate( " + ( width * 0.3 * i ) + "," + ( margin * 0.6 ) + " )";
            } );
        ...
        legend.append( "text" )
                .attr( "x", function( d, i ) {
                    return ( margin * 0.30 + legendWidth );
                })
                .attr( "y", yValueOfText )
                .attr( "dy", ".35em" )
...
function plotTheChartCointer() {
    ...
    //Appening X-Axis For Grouped Bar Chart
        ...
        svgForLineChart = creatingSvgForChart( mainSvg, lineChartHeight, width, [ margin, ( height + margin - lineChartHeight ) ], "svgForLineChart" ),
...
function sortDataAndReplotTheChart( data, sortValue ) {
    ...
    xScale.domain( countryCode );
    xScaleForGroupedBarChart.domain( countryCode );
    color.range( [ "#527B34" ,"#4D951A", "#49B000" ] );
    }
